...
//
//   let gttGetOrder = try GttGetOrder(json)

//
// To read values from URLs:
//
//   let task = URLSession.shared.gttGetOrderTask(with: url) { gttGetOrder, response, error in
//     if let gttGetOrder = gttGetOrder {
//       ...
//     }
//   }
//   task.resume()
...
//
//   Alamofire.request(url).responseGttGetOrder { response in
//     if let gttGetOrder = response.result.value {
//       ...
//     }
//   }
...
// MARK: - GttGetOrder
struct GttGetOrder: Codable {
    let data: DataClass?
    let status: String?
}

// MARK: GttGetOrder convenience initializers and mutators

extension GttGetOrder {
    init(data: Data) throws {
        self = try newJSONDecoder().decode(GttGetOrder.self, from: data)
    }
...
extension URLSession {
    ...
    func gttGetOrderTask(with url: URL, completionHandler: @escaping (GttGetOrder?, URLResponse?, Error?) -> Void) -> URLSessionDataTask {
...
extension DataRequest {
    ...
    @discardableResult
    func responseGttGetOrder(queue: DispatchQueue? = nil, completionHandler: @escaping (DataResponse<GttGetOrder>) -> Void) -> Self {
