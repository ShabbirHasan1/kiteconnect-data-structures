...
//
//   let marginsEquity = try MarginsEquity(json)

//
// To read values from URLs:
//
//   let task = URLSession.shared.marginsEquityTask(with: url) { marginsEquity, response, error in
//     if let marginsEquity = marginsEquity {
//       ...
//     }
//   }
//   task.resume()
...
//
//   Alamofire.request(url).responseMarginsEquity { response in
//     if let marginsEquity = response.result.value {
//       ...
//     }
//   }
...
// MARK: - MarginsEquity
struct MarginsEquity: Codable {
    let data: DataClass?
    let status: String?
}

// MARK: MarginsEquity convenience initializers and mutators

extension MarginsEquity {
    init(data: Data) throws {
        self = try newJSONDecoder().decode(MarginsEquity.self, from: data)
    }
...
extension URLSession {
    ...
    func marginsEquityTask(with url: URL, completionHandler: @escaping (MarginsEquity?, URLResponse?, Error?) -> Void) -> URLSessionDataTask {
...
extension DataRequest {
    ...
    @discardableResult
    func responseMarginsEquity(queue: DispatchQueue? = nil, completionHandler: @escaping (DataResponse<MarginsEquity>) -> Void) -> Self {
